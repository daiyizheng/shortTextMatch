...
# @Author : daiyizheng
# @Version：V 0.1
# @File : xlnet_modeling.py
# @desc :

import torch
import torch.nn as nn
from transformers import XLNetModel, XLNetPreTrainedModel, BertPreTrainedModel

from src.DL_model.bert_models.layers.classifier_layer import Classifier

class ClsXLNet(XLNetPreTrainedModel):
    def __init__(self, config, args):
        super(ClsXLNet, self).__init__(config)
        self.args = args
        self.config = config
